...
**Протокол снят живьём 2026-08-15 (полный диапазон):**
- Часы: `R0 = 0xA0 | h`, 1ч=`A1` … 24ч=`B8` (декод `R0 & 0x1F`), выкл=`R0=00`.
- Кадры Timer+ несут **cmd `0x03`** (при R0≠0) с реальным состоянием в R3/R4.
  Гипотеза: R0 — селектор поля (`00`=температура, `0xA0|h`=таймер) для cmd 02/03.
- Одиночный кадр `0x0D` (R0=A4, R1 бит4) — вероятно, кнопка Timer (вход/выход
  в режим настройки таймера).
- **Открытый вопрос:** как отличить «timer off» (0x03 R0=00) от «temp−» (0x03 R0=00?).
  Нужен мини-захват в COOL: Temp+, Temp−, Timer ×2.
- Логирование: `timer: cmd=.. R0=.. -> Nh` по R0 при любой команде; `0x0D` кадры
  логируются отдельно (`timer-mode:`) и не меняют состояние climate.
- Реализация: `set_timer(hours)` + number-сущность — после уточняющего захвата.
