...
## Этап 11 — ESP внутри кондиционера (полный hvac_action) — идея 2026-08-15

Задача: получить **реальное** состояние кондиционера (уставка/режим/fan/компрессор/
ошибки/комнатная t°) не через ИК-эхо, а напрямую из электроники indoor-unit.

**Архитектура сплита:** main board ↔ display board кабелем на 3-5 проводов;
главная плата сама говорит дисплею что показывать (= полное состояние).
Прецеденты: TCL/Kesser — UART 9600 8E1, кадры 0xBB..XOR (github.com/adaasch/AC-hack),
LG — UART + ESP32 (flameeyes.blog), TCL IDU↔ODU (dj9kw.de), Midea local serial
(HA community). На display board обычно сидят IR-приёмник и термистор комнатной t°.

**Решение — параллельный врез (НЕ замена платы дисплея):**
- IR-линия приёмника → ESP читает параллельно (пульт ловится изнутри)
- Термистор t° → ADC ESP (TYWE3S имеет TOUT) → честный current_temperature
- Data-линия дисплея → сниф UART → декод полного состояния
- Питание +5В с main board (у YTF свой LDO)
- Кандидат: сам YTF bridge внутрь корпуса — ноль нового железа

**Фазы:**
1. Разведка: фото display board + разъёма, счёт проводов, вольтаж пинов, найти
   IR-приёмник/термистор. (Фото анализируем в чате.)
2. Сниф data-линии (2400/4800/9600, 8N1/8E1), дампы в разных режимах → декод.
3. Монтаж: YTF внутрь, IR параллельно, термистор на ADC, при успехе — full state.

Статус: ждём фото платы от пользователя.
